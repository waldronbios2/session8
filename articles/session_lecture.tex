...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(pensim); }\KeywordTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{mydat <-}\StringTok{ }\KeywordTok{create.data}\NormalTok{(}\DataTypeTok{nvars=}\KeywordTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{1}\NormalTok{), }\DataTypeTok{nsamples=}\DecValTok{500}\NormalTok{, }
    \DataTypeTok{cors=}\KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{), }\DataTypeTok{associations=}\KeywordTok{c}\NormalTok{(}\FloatTok{0.5}\NormalTok{, }\FloatTok{0.5}\NormalTok{), }
    \DataTypeTok{firstonly=}\KeywordTok{c}\NormalTok{(}\OtherTok{TRUE}\NormalTok{, }\OtherTok{TRUE}\NormalTok{), }\DataTypeTok{censoring=}\KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\FloatTok{8.5}\NormalTok{))}\OperatorTok{$}\NormalTok{data}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{colnames}\NormalTok{(mydat)[}\DecValTok{1}\OperatorTok{:}\DecValTok{2}\NormalTok{] <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Var1"}\NormalTok{, }\StringTok{"Var2"}\NormalTok{)}
\NormalTok{mydat}\OperatorTok{$}\NormalTok{Var1 <-}\StringTok{ }\KeywordTok{cut}\NormalTok{(mydat}\OperatorTok{$}\NormalTok{Var1, }\DataTypeTok{breaks=}\DecValTok{2}\NormalTok{, }\DataTypeTok{labels=}\KeywordTok{c}\NormalTok{(}\StringTok{"low"}\NormalTok{, }\StringTok{"high"}\NormalTok{))}
\NormalTok{mydat}\OperatorTok{$}\NormalTok{time <-}\StringTok{ }\KeywordTok{ceiling}\NormalTok{(mydat}\OperatorTok{$}\NormalTok{time}\OperatorTok{*}\DecValTok{1000}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
